...
const CartItem = (props) => {
    ...
    return (
        <div className="cartItem">
            <div className="cartImg"><img src={image} alt={data.name}/></div>
            <div className="cartDetails">
                <div className="cartName">{data.name} <b>${data.price}</b></div>
                <div className="cartDesc">{data.description}</div>
                <div className="cartQuantity">
                {cart[data.id].quantity > 1 && <div className="cartDown" onClick={() => cartDown()}>{"<"}</div>}
                {cart[data.id].quantity === 1 && <div className="cartDownNull"></div>}
                    <div className="cartNumber">{cartItem.quantity}</div>
                    <div className="cartUp" onClick={() => cartUp()}>{">"}</div>
                </div>
                <div className="cartRemove" onClick={() => cartRemove()}>remove</div>
            </div>
        </div>
    )
}
